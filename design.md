...
3. **临时 string 优化是否值得单开** — `find(const char*)` 每次构造 `std::string`（~900ns debug 大头）；修法（transparent hash / uint32 key）改动 cache 内部类型，属未来独立 polish 项，不在 P3 范围。
