...
> **2026-09-21 曲线采样补充**：正式 `KeySampler` 与 `AnimationPlayer` 已消费 `.ayanm` v5 的 Linear、Step、Cubic Hermite 插值及逐关键帧入/出切线。Float、Vector3 使用标准 Hermite；Quaternion 进行最短弧端点处理、分量 Hermite 后归一化。旧 v1–v4 动画保持 Linear 行为。普通骨架重命名烘焙完整保留曲线；跨骨架位置/缩放切线按转换尺度变换，Cubic Quaternion 在尚无切空间转换时显式阻止。
